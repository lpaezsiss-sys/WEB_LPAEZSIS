#!/usr/bin/env python3
"""
Idempotent MOVEX Blueline® brand refresh + modular-belt quote product.
...
Sends canonical BrandSeo fields plus legacy aliases (description / content_html).
  ...
  ADMIN_PASSWORD='…' python3 tools/import_movex_blueline.py
  ADMIN_TOKEN='…'    python3 tools/import_movex_blueline.py
"""
...
CATALOG_PATH = ROOT / "data" / "movex_blueline_catalog.json"
...
def import_live(catalog: dict, api: str, password: str | None, token: str | None) -> None:
    ...
    for prod in catalog["products"]:
        ...
        img_file = ROOT / "site" / str(image_url).lstrip("/")
        if img_file.is_file():
            print("canonical product image", image_url, img_file)
        else:
            print("warn: falta archivo de producto", img_file)

        ds = str(prod.get("datasheet") or "")
        if ds.lower().endswith(".pdf"):
            ds_file = ROOT / "site" / ds.lstrip("/")
            if ds_file.is_file() and not remote_ok(client.base, ds):
                try:
