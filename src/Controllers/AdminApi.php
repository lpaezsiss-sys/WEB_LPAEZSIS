...
final class AdminApi
{
    public static function handle(string $method, string $path): void
    {
        ...
        // Sectores (home "Sectores donde trabajamos")
        if ($method === 'GET' && $sub === '/sectores') {
            PublicApi::ensureSectoresSchema();
            $rows = self::pdo()->query(
                'SELECT * FROM sectores ORDER BY orden ASC, nombre ASC'
            )->fetchAll();
            Response::json(['sectores' => $rows]);
            return;
        }
        if ($method === 'POST' && $sub === '/sectores') {
            self::createSector();
            return;
        }
        if ($method === 'PUT' && preg_match('#^/sectores/(\d+)$#', $sub, $m)) {
            self::updateSector((int) $m[1]);
            return;
        }
        // POST multipart: actualizar + imagen en un solo FormData
        if ($method === 'POST' && preg_match('#^/sectores/(\d+)$#', $sub, $m)) {
            self::saveSectorForm((int) $m[1]);
            return;
        }
        if ($method === 'POST' && preg_match('#^/sectores/(\d+)/imagen$#', $sub, $m)) {
            self::uploadSectorImagen((int) $m[1]);
            return;
        }
        if ($method === 'DELETE' && preg_match('#^/sectores/(\d+)$#', $sub, $m)) {
            self::deleteSector((int) $m[1]);
            return;
        }
    ...
    private static function createSector(): void
    {
        PublicApi::ensureSectoresSchema();
        $b = self::body();
    ...
    private static function updateSector(int $id): void
    {
        PublicApi::ensureSectoresSchema();
        $b = self::body();
        if (array_key_exists('activo', $b)) {
        ...
        $fields = ['nombre', 'slug', 'imagen_url', 'link_url', 'orden', 'activo'];
        $sets = [];
        $vals = [];
        foreach ($fields as $f) {
            if (!array_key_exists($f, $b)) {
        ...
        $vals[] = $id;
        self::pdo()->prepare('UPDATE sectores SET ' . implode(', ', $sets) . ' WHERE id = ?')->execute($vals);
        $stmt = self::pdo()->prepare('SELECT * FROM sectores WHERE id = ?');
        $stmt->execute([$id]);
        Response::json($stmt->fetch() ?: ['ok' => true]);
    }

    /**
     * POST multipart /sectores/{id}: actualiza campos + sube imagen a img/uploads/sectores/.
     */
    private static function saveSectorForm(int $id): void
    {
        PublicApi::ensureSectoresSchema();
        $stmt = self::pdo()->prepare('SELECT * FROM sectores WHERE id = ? LIMIT 1');
        $stmt->execute([$id]);
        $existing = $stmt->fetch();
        if (!$existing) {
        ...
        self::pdo()->prepare(
            'UPDATE sectores SET nombre = ?, slug = ?, imagen_url = ?, link_url = ?, orden = ? WHERE id = ?'
        )->execute([$nombre, $slug, $imagen, $link, $orden, $id]);

        $out = self::pdo()->prepare('SELECT * FROM sectores WHERE id = ?');
        $out->execute([$id]);
        Response::json($out->fetch() ?: ['ok' => true]);
    }

    private static function uploadSectorImagen(int $id): void
    {
        PublicApi::ensureSectoresSchema();
        $stmt = self::pdo()->prepare('SELECT id FROM sectores WHERE id = ? LIMIT 1');
        $stmt->execute([$id]);
        if (!$stmt->fetch()) {
            Response::error('Sector no encontrado', 404);
            return;
    ...
    private static function deleteSector(int $id): void
    {
        PublicApi::ensureSectoresSchema();
